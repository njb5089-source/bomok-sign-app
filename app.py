...
def generate_announcement_with_ai(title, date, location, supplies, extra_info,
    ...
    try:
        ...
        - 기타 강조사항: {extra_info}
        {privacy_block}
        [반드시 빠짐없이 포함해야 할 법적 고지 — 아래 4가지 중 하나라도 누락하면 안 됨]
        본문 끝에 '개인정보 처리 및 동의 안내' 부분을 만들어, 다음을 정확하고 명확하게 모두 포함할 것:
        1) (법정대리인 동의) 만 14세 미만 아동의 개인정보 처리는 「개인정보 보호법」 제22조의2에 따라 법정대리인(보호자)의 동의가 필요하며, 본 동의서는 법정대리인인 보호자가 동의하는 것임을 명확히 안내.
        2) (수집·이용 및 파기 고지) 수집하는 개인정보 항목과 이용 목적, 보유·이용 기간을 안내하고, 보유기간이 끝나거나 처리 목적을 달성하면 해당 개인정보를 지체 없이 안전하게 파기한다는 점을 반드시 명시.
        3) (동의 거부권) 정보주체(법정대리인)는 개인정보 수집·이용 동의를 거부할 권리가 있으며, 다만 동의를 거부할 경우 프로그램 참여가 제한될 수 있음을 안내.
        4) (민감·고유식별정보) 주민등록번호 등 고유식별정보나 건강정보 등 민감정보를 수집하는 경우, 관련 법령에 근거하여 별도로 안전하게 관리하며 수집 목적 외로 이용하지 않음을 안내.

        부드러운 해요체(~합니다, ~바랍니다)를 쓰되, 위 법적 고지 4가지는 정확하고 명확하게 작성해줘. 이모지와 줄바꿈으로 읽기 쉽게.
        """
