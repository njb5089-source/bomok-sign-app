...
{info_lines}
        ...
        본문 맨 마지막은 편지처럼 마무리해줘. 먼저 발행일을 '○○○○년 ○월 ○일' 형식으로 한 줄 쓰고,
        그 아래 줄에 발행 주체 이름에 '드림'을 붙여 적어줘. (예: 2026년 7월 1일 → 다음 줄 → 보목지역아동센터 드림)
        발행일과 발행 주체는 본문 중간에 따로 나열하지 말고 반드시 이 마무리에만 배치해줘.
...
(function(){
  ...
  function allBtns(){ return Array.prototype.slice.call(doc.querySelectorAll('[data-testid="stTabs"] button[role="tab"]')); }
  function subBtns(){ return allBtns().filter(function(b){ var t=b.textContent; return t.indexOf('작성')>=0||t.indexOf('시안')>=0||t.indexOf('발송')>=0; }); }
  function activeIdx(){ var t=subBtns(); for(var i=0;i<t.length;i++){ if(t[i].getAttribute('aria-selected')==='true') return i; } return 0; }
  function toTop(){
  ...
  prev.onclick=function(){ var t=subBtns(), i=activeIdx(); if(i>0){ t[i-1].click(); toTop(); } };
  var next=mkBtn('floatNext','›','right:6px;top:50%;transform:translateY(-50%);');
  next.onclick=function(){ var t=subBtns(), i=activeIdx(); if(i<t.length-1){ t[i+1].click(); toTop(); } };
  // 탭을 직접 클릭해도 항상 맨 위에서 시작
  allBtns().forEach(function(b){ if(!b.dataset.tt){ b.dataset.tt='1'; b.addEventListener('click', function(){ setTimeout(toTop, 60); }); } });
})();
</script>
"""
...
def goto_tab_js(label):
    """JS로 라벨이 포함된 탭 버튼을 클릭(중첩 탭 대응)하고 맨 위로 스크롤합니다."""
    safe = str(label).replace("'", "").replace('"', "")
    js = """
<script>
setTimeout(function(){
  var d = window.parent.document;
  var btns = d.querySelectorAll('[data-testid="stTabs"] button[role="tab"]');
  btns.forEach(function(b){ if (b.textContent.indexOf('LABEL') >= 0) b.click(); });
  setTimeout(function(){
...
# =====================================================================
    ...
    main_pub, main_manage = st.tabs(["📝 새 안내문 발행", "📂 안내문 관리·제출현황"])
    ...
    with main_pub:
        s1, s2, s3 = st.tabs(["1️⃣ 작성", "2️⃣ 시안 확인·수정", "3️⃣ 발송"])
        with s1:
            ...
            if st.button("🪄 AI 안내문 초안 자동 생성하기"):
                with st.spinner("Gemini AI가 멋진 가정통신문을 작성하고 있습니다..."):
                    ...
                    st.session_state.goto_tab = "시안"   # 완료 후 시안 탭으로 자동 이동
                    st.rerun()
        ...
        with s2:
            if True:
                ...
                if st.button("🚀 시안 확정 및 발송 링크 생성"):
                    ...
                    st.session_state.goto_tab = "발송"   # 발행 후 발송 탭으로 자동 이동
        ...
        with s3:
            if not st.session_state.get('generated', False):
                st.info("'시안 확인·수정' 탭에서 '시안 확정 및 발송 링크 생성'을 누르면 여기에 링크가 나옵니다.")
    ...
    with main_manage:
        st.write("### 📂 발행한 안내문 관리")
        st.caption("발행한 안내문을 선택하면 그 안내문의 내용과 제출 현황을 볼 수 있습니다.")
        _anns = load_all_announcements()
        _subs = load_submissions() or []
        if _anns is None:
            st.error("안내문 목록을 불러오지 못했습니다. (시트 연결 확인)")
        elif not _anns:
            st.info("아직 발행한 안내문이 없습니다. '새 안내문 발행' 탭에서 작성하세요.")
        else:
            _titles = [f"{x.get('title','(제목 없음)')}  ·  {x.get('issue_date','')}" for x in _anns]
            _sel = st.radio("📑 안내문 목록 (선택)", range(len(_anns)),
                            format_func=lambda i: _titles[i], key="manage_sel")
            a = _anns[_sel]
            aid = str(a.get("id", ""))
            _rows = [s for s in _subs if str(s.get("안내문ID")) == aid]
            m1, m2 = st.tabs(["📄 안내문 내용", f"📋 제출 현황 ({len(_rows)})"])
            with m1:
                st.markdown(f"### 🌲 {a.get('title','')}")
                if a.get("issue_date"):
                    st.caption(f"발행일: {a['issue_date']}")
                st.info(a.get("desc", ""))
                if a.get("collection"):
                    st.markdown("**📑 개인정보 수집·이용 내역**")
                    st.table([
                ...
                st.link_button("🔗 학부모 화면 열어보기", _alink, use_container_width=True)
                link_copy_widget(_alink)
                if st.button("🗑 이 안내문 삭제", key=f"mdel_{aid}"):
                    delete_announcement(aid)
                    st.cache_data.clear()
                    st.rerun()
            with m2:
                if not _rows:
                    st.info("아직 이 안내문에 제출된 동의서가 없습니다.")
                else:
                    st.success(f"총 {len(_rows)}건 제출")
                    for rec in reversed(_rows):
                        child = rec.get("아동 성명", "")
                        guardian = rec.get("보호자 성명", "")
                        when = rec.get("제출시각", "")
                        with st.expander(f"🧾 {when} · {child} (보호자: {guardian})"):
                            for k, v in rec.items():
                                if k in ("서명이미지", "안내문ID") or not str(v).strip():
                                    continue
                                st.write(f"- **{k}**: {v}")
                            sig = rec.get("서명이미지", "")
                            if sig:
            ...
            if st.button("🔄 새로고침", key="manage_refresh"):
                st.cache_data.clear()
                st.rerun()
